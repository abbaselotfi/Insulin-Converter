...
function populateInsulins() {
  ["sourceInsulin","targetInsulin"].forEach((id,index) => {
    const select = document.getElementById(id);
    select.innerHTML = ClinicalEngine.INSULINS.map((item,i) => `<option value="${item.id}" ${i===index?"selected":""}>${insulinLabel(item)}</option>`).join("");
  });
}
...
$("#conversionForm").addEventListener("submit",(event)=>{
  ...
  try {
    const result=ClinicalEngine.calculateConversion({sourceId:$("#sourceInsulin").value,targetId:$("#targetInsulin").value,dailyDose:$("#currentDose").value,factor:$("#strategy").value});
    $("#resultDose").textContent=result.estimatedDose; $("#calculationText").textContent=result.formula;
    $("#resultSource").textContent=insulinLabel(result.source); $("#resultTarget").textContent=insulinLabel(result.target); $("#resultFactor").textContent=`× ${result.factor}`;
