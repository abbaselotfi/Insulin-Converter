const translations = {
  fa: {
    ...
    resultWaiting: "نتیجه اینجا نمایش داده می‌شود", resultWaitingText: "اطلاعات رژیم فعلی و مقصد را تکمیل کنید.",
  ...
  en: {
    ...
    resultWaiting: "Your result will appear here", resultWaitingText: "Complete the current and target regimen details.",
    estimatedDose: "Estimated starting dose", unitsPerDay: "units per day", from: "From", to: "To", factor: "Factor",
...
function therapyLabel(therapy) {
  return therapy.category === "frc" ? therapy.brand : `${therapy.generic} — ${therapy.brand}`;
}
...
function therapyOptions(items) {
  return Object.entries(categoryLabels()).map(([category, label]) => {
    const options = items
      ...
      .map((item) => `<option value="${item.id}">${therapyLabel(item)}</option>`)
      .join("");
...
function showError(errorCode) {
  ...
  const box = $("#formError");
  box.textContent = translations[language][keyByCode[errorCode] || "invalidDose"];
...
function renderComposition(result) {
  const compositions = [
    result.sourceComposition && { context: "مبدأ", value: result.sourceComposition },
    result.targetComposition && { context: "مقصد", value: result.targetComposition }
  ].filter(Boolean);
  $("#compositionResult").hidden = compositions.length === 0;
  $("#compositionText").textContent = compositions.map(({ context, value }) =>
    `${context}: ${value.basalPercent}٪ بیزال (${value.basalDose} واحد) + ${value.prandialPercent}٪ پرندیال (${value.prandialDose} واحد)`
  ).join(" | ");
}

function renderSchedule(result) {
  $("#scheduleResult").hidden = !result.schedule;
  $("#scheduleText").textContent = result.schedule
    ? result.schedule.map((item) => `تزریق ${item.injection}: ${item.dose} واحد`).join(" | ")
    : "";
}
...
function renderResult(result) {
  ...
  $("#penResult").hidden = !result.soliqua;
  $("#penResult").className = `pen-result ${result.soliqua?.colorClass || ""}`;
  $("#penResult").textContent = result.soliqua
    ? `FRC Soliqua ${result.soliqua.pen} — قلم ${result.soliqua.colorName} (${result.soliqua.penRange} dose steps)`
    : "";
